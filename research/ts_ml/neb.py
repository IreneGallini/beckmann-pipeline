...
connectivity check. Same verification shape as ts_ml.ts.verify_ts (one
...
Reuses geometries from ts_ml.ts_products (reactant from
...
ts_ml.ts's own TS1_B1/TS2_B1 split.
...
def mol_to_xyz(mol: Chem.Mol, title: str) -> str:
    """Same simple XYZ format as research/analysis_scripts/classical_benchmark.py's
    own mol_to_xyz -- not imported from there, to avoid a cross-directory
...
def verify_ts_ml(hessian_h5_path: Path, atoms_of_interest: dict[str, int],
    ...
    Does NOT auto-decide pass/fail, same precedent as ts_ml.ts.verify_ts.
...
def main() -> None:
    ...
    from beckmann_nbo.descriptors import _load_mols, get_substituent_map
