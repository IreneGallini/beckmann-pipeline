...
- `data/` -- archived/side-experiment Gaussian output not part of the
  canonical 34-molecule benchmark set. Currently just
  `dft_opt_gasphase_archive/` (gas-phase reruns for mol_002/006/020/021 --
  no written-up conclusion exists yet, so this is the only record of
  whatever that investigation found; keep until it's written up or
  confirmed abandoned) and `analysis/archive_pre_rigidscan_2026-07-15/`.
  Two other side-experiments that lived here (a basis-set sensitivity
  check, a finer-resolution scan rerun) were deleted once their findings
  were confirmed to already be fully documented in `Notes.md`/`CLAUDE.md`.
  The canonical data (`dft_opt/`, `dft_sp/`, `aimnet_optimized/`,
  `conformers/`, the non-archived parts of `analysis/`) stays in the shared
  top-level `data/`, since `beckmann-nbo`'s own scripts read from there too.
- `tests/` -- integration tests for `benchmark_pipeline/`'s generated
  artifacts; not unit tests for `beckmann-core`'s own functions (see
  `packages/beckmann-core/tests/` for those).
