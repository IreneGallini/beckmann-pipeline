...
series (data/output/analysis/cmo_channel_extraction.csv) and the open-source
per-atom-pair series (data/output/analysis/wcnmax_channel_extraction_opensource.csv,
beckmann_alt.pair_nbo). One PNG per molecule present in the open-source extraction
CSV (32 of 34 -- mol_005_E/mol_032_E permanently skipped, bromine, see
Notes_open_source_alt.md). Originally written for mol_006_E only (the one molecule in
...
rearrangement, matching experiment; the open-source series didn't), generalized here
to the full covered set.

Output: data/output/analysis/plots/mol{NNN}_opensource_vs_nbo7_wcnmax.png
"""
import csv
...
def plot_comparison(mol: str, analysis_dir, plots_dir) -> None:
    ...
    if trusted_xs:
        ...
        print(f"-- {mol}: no trusted NBO7 'cn' data, plotting open-source only")
    ax.plot(
    ...
    out_path = plots_dir / f"mol{mol_id}_opensource_vs_nbo7_wcnmax.png"
...
def main() -> None:
    ...
    mols = sorted({
        row["mol"] for row in csv.DictReader(open(analysis_dir / "wcnmax_channel_extraction_opensource.csv"))
    })
    for mol in mols:
