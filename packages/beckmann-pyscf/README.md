...
Each job (Auto3D + AIMNet2 + a 7-point PySCF scan) runs in its own OS process, not a thread, which is what lets a stuck/slow job actually be terminated on timeout instead of just being reported as timed out while it keeps burning CPU. See `backend/jobs.py`'s module docstring for the full design.
