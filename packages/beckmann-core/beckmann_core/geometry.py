"""
Pure geometry primitives shared by both downstream products -- operate
directly on (sym, x, y, z) tuples, no Gaussian-log parsing or other
method-specific I/O. (The Gaussian-log-format-specific half of the original
beckmann/dft/geometry.py, parse_standard_orientations(), stays in
beckmann-nbo -- it's not something a PySCF/AIMNet2-only pipeline ever needs,
since that path builds atom tuples directly from RDKit conformers.)
"""
import math
...
def displace_leaving_group(atoms: list, ni: int, oi: int, delta: float) -> list:
    ...
    independently from a single fixed base structure, rather than chaining
    from the previous point or extracting from an internal Gaussian scan
    walk -- see the rigid-scan architecture notes (beckmann-nbo's
    RIGID_SCAN_MIGRATION.md) for why. Method-agnostic: used both by
    beckmann-nbo's Gaussian rigid-scan .gjf generation and by
    beckmann-pyscf's own AIMNet2-relaxed scan points.
    """
