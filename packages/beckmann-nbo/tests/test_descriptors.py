"""
Tests for beckmann_nbo/descriptors.py (aryl/alkyl role tagging, Psi, d/dR slopes).
"""
import pytest

from beckmann_nbo.descriptors import get_substituent_map, least_squares_slope
...
def dft_opt_dir(data_output):
    ...
    if not p.exists():
        pytest.skip("dft_opt/ not found -- run beckmann-nbo's prepare_opt.py first")
    return p
