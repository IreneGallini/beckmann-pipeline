...
The recommended entry point for a new user, wrapping this package's underlying modules so you can go from a SMILES string to a prediction without reading the full workflow below. Every subcommand is a thin wrapper: no pipeline logic lives in the CLI layer itself.
