...
def cmd_submit_scan_sp(config: dict, dry_run: bool, mol: str | None, local_dir: Path) -> None:
    """Submit intermediate scan single-point NBO jobs (*_sp2.gjf, *_sp3.gjf, *_sp4.gjf).

    Generated by scripts/dft/extract_scan_sp.py from the converged scan geometries
    at R0+0.1, R0+0.2, R0+0.3 Å.  Run after the Stage 3 scan is complete.
    """
    host       = config["HPC_HOST"]
    remote_dir = config["HPC_REMOTE_DIR"]
    pattern    = f"mol_{mol.zfill(3)}_*" if mol else "*"
...
def main() -> None:
    ...
    sub.add_parser("submit-scan-sp", help="Submit intermediate scan SP jobs ({name}_sp2/3/4.gjf) — AFTER scan finishes")
